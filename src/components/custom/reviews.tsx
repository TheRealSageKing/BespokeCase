...
function ReviewColumn({ reviews, className, reviewClassName, msPerPixel = 0 }: { reviews: string[]; className?: string; reviewClassName?: (reviewIndex: number) => string; msPerPixel?: number }) {
	...
	return (
		<div ref={columnRef} className={cn("animate-marquee space-y-8", className)} style={{ "--marquee-duration": duration } as React.CSSProperties}>
...
function Review({ imgSrc, className, ...props }: ReviewProps) {
	...
	return (
		<div className={cn("animate-fade-in rounded-[2.25rem] bg-white p-6 opacity-0 shadow-xl shadow0slate900/5", className)} style={{ animationDelay }} {...props}>
			<Phone imgSrc={imgSrc} />
		</div>
	);
}
			...
			{isInView ? (
				<>
					<ReviewColumn
						...
						msPerPixel={10}
					/>
					<ReviewColumn reviews={[...column2, ...column3[1], ...column2]} reviewClassName={(reviewIndex) => (reviewIndex >= column2.length ? "lg:hidden" : "")} className="hidden md:block" msPerPixel={15} />
...
export default function Reviews() {
	return (
		<MaxWidthWrapper className="relative max-w-5xl">
			<img src="/what-people-are-buying.png" alt="review 1" className="absolute select-none hidden xl:block -left-32 top-1/3" />
			<ReviewGrid />
		</MaxWidthWrapper>
	);
}
