...
import HandleComponent from "@/components/custom/handle-components";
	...
	async function saveConfiguration() {
		startTransition(async () => {
			try {
				...
				userImage.src = imageUrl; //user image url from where we uploaded it to
				await new Promise((resolve) => (userImage.onload = resolve)); //load image from src
				...
				console.log("was clicked", base64);
				...
				toast({
					title: "Something went wrong",
					description:
						"There was a problem. Your design waas not saved, please try again",
					variant: "destructive",
				});
			}
		});
			...
			<div className="h-[37.5rem] w-full col-span-full lg:col-span-1 flex flex-col bg-white">
				<ScrollArea className="relative flex-1 overflow-auto">
					...
					<div className="px-8 pb-12 pt-8">
						...
						<div className="w-full h-px background-zinc-200 my-6" />
										...
										<DropdownMenuContent>
											{MODELS.options.map((model) => (
												<DropdownMenuItem
													key={model.label}
													className={cn(
														"flex text-sm gap-1 items-center p-1.5 cursor-default hover:bg-zin-100",
														{
															"bg-zinc-100":
											...
											<div className="mt-3 space-y-4">
												{selectableOptions.map((option) => (
													<Radio
														...
														className={({ checked }) =>
															cn(
																"relative block cursor-pointer rounded-lg bg-white px-6 py-4 shadow-sm border-2 border-zinc-200 focus:outlune-none ring-0 focus:ring-0 sm:flex sm:justify-between outline-none",
																{
					...
					<div className="w-full h-full flex justify-end items-center">
						<div className="w-full flex gap-6 items-center">
							...
							<Button
								...
								isLoading={isPending}
								disabled={isPending}
								loadingText="Generating your design">
